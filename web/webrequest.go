...
func (wp *WebProxy) GetShipInfo(id string) ([]byte, error) {
	return wp.get(fmt.Sprintf(httpEndpointGetShipDetails, id, wp.token))
}
...
func (wp *WebProxy) SetNewFlightPlan(id, destination string) ([]byte, error) {
	return wp.post(
		fmt.Sprintf(httpEndpointPostFlightPlanNew, wp.token),
		bytes.NewReader(
			[]byte(fmt.Sprintf("{\"shipId\": \"%s\", \"destination\": \"%s\"}", id, destination))))
}
...
func (wp *WebProxy) BuyGood(id, good string, quantity int) ([]byte, error) {
	return wp.post(
		fmt.Sprintf(httpEndpointPostBuyOrderNew, wp.token),
		bytes.NewReader(
			[]byte(
				fmt.Sprintf("{\"shipId\": \"%s\", \"good\": \"%s\", \"quantity\": %d}", id, good, quantity))))
}

// SellGood sends to game a sell order.
//
// https://api.spacetraders.io/#api-sell_orders-NewSellOrder
func (wp *WebProxy) SellGood(id, good string, quantity int) ([]byte, error) {
	return wp.post(
